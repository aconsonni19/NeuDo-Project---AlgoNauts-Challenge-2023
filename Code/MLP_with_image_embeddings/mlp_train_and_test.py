"""
This file contains the functions used to extract features with CLIP, train and test the model and plot the results
"""
import gc
import glob
import os.path
...
# =============================================================================
# Algorithm to extract features using the CLIP model
# =============================================================================
def extract_clip_features(sub, clip_model_name, project_dir, output_dir):
    ...
    centre_crop = trn.Compose([
        trn.Resize((224, 224)),  # Resize the input image to a fixed size of 224x224 pixels
        trn.ToTensor(), # Convert the image to a tensor  and scale pixel values to [0, 1]
        # Normalize the tensor using ImageNet mean and standard deviation
        trn.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])

    # If a CUDA device is available, utilize it as the main computation device instead of the CPU
    ...
    # Loop on both the training and test split of the dataset
    for s in range(len(splits_parent)):
        # Get the list of train/test images
        ...
        # Extract the features maps from the images
        for i in range(0, len(image_list), batch_size):
            ...
            for image in tqdm(batch_images, desc=f"Batch {i // batch_size + 1}"):
                ...
                # Crop image to center and adds dimension for batch processing
                ...
                # Append image features to the batch features list
...
def train_mlp_models(sub, project_dir, output_dir, trials = 5):
    """
    Train the MLP model on the CLIP feature embedding extracted. The
    model's hyperparameters (learning rate and weight decay)
    will be tuned with a random search tuning strategy
    :param sub: The subject utilized
    :param project_dir: The project directory where the Datases is contained
    :param output_dir: The project Output directory, where the Outputs of the model are contained
    :param trials: Number of hyperparameter tuning trials
    :return:
    """
    # =============================================================================
    # Initialization
    ...
    torch.cuda.empty_cache() # Free computation resources withheld from previous steps
    ...
    for trial in range(trials):
        ...
        for model in mlp.get_all_available_mlp(input_dim, output_dim):
            ...
            # Initialize optimizer function with current random lr and weight_decay
            optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
            # Initialize scheduler, which will halve the LR every 3 iterations without improvement of loss function on
            # training set
